...
M1–M3 unchanged: atlas (`projection.ts`, `MorphBorders.ts`, `AtlasMap`/`AtlasFrame`/`AtlasInsets`/`AtlasFilters`, four eras, hover/morph paint fix) and the cloud sweep (`CloudSweep.tsx`, `transitionStore.ts`, `sweepTween.ts`) are exactly as M2/M3 left them. `engine/` still never imports `content/`. Vitest + Playwright green (26 e2e, 108 unit — up from 17/60). Next: M5 — Post-processing and particles.
